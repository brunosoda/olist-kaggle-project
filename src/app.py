...
date_filter_type = st.sidebar.selectbox("Filter by which date?", list(date_type_map.keys()))
...
selected_states = st.sidebar.multiselect("State (Destination)", states)
...
selected_cities = st.sidebar.multiselect("City (Destination)", cities)

# 5. Category Filter
categories = sorted(df_raw['product_category_name'].dropna().unique())
selected_categories = st.sidebar.multiselect("Product Category", categories)
